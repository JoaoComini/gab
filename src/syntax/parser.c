...
static ASTExpr *parse_primary(Parser *parser) {
    ...
    case TOKEN_INT: {
        ...
        return ast_literal_expr_create(parser->arena, span, (Literal){.kind = LITERAL_INT, .as_int = value});
    }
    case TOKEN_FLOAT: {
        ...
        return ast_literal_expr_create(parser->arena, span,
                                       (Literal){.kind = LITERAL_FLOAT, .as_float = value});
    }
    case TOKEN_STRING: {
        ...
        return ast_literal_expr_create(parser->arena, span,
                                       (Literal){.kind = LITERAL_STRING, .as_string = text});
    }
    case TOKEN_TRUE: {
        parser_next_token(parser);

        return ast_literal_expr_create(parser->arena, span, (Literal){.kind = LITERAL_BOOL, .as_bool = true});
    }
    case TOKEN_FALSE: {
        parser_next_token(parser);
