...
static void test_a_parameter_sits_where_the_caller_left_it(void) {
    Allocated a;
    allocate(&a, "func f(x: int, y: int): int { return x + y; }");
...
static void test_no_two_values_live_at_once_share_a_slot(void) {
    static const char *const sources[] = {
        "func f(n: int): int { let a = 0; for let i = 0; i < n; i = i + 1 { a = a + i; } return a; }",
        "func f(c: bool, n: int): int { if c { return n + 1; } return n + 2; }",
        "func f(n: int): int { let a = n * 2; let b = a + 1; let c = b + a; return c; }",
        "struct P { x: int }\nfunc f(p: P): int { let q = P { x: p.x }; return q.x; }\n"};
...
static void test_a_frame_too_large_is_reported_rather_than_wrapped(void) {
    ...
    at += (size_t)snprintf(source + at, sizeof(source) - at,
                           "last: int }\n"
                           "func f(a: Wide, b: Wide): int { return 0; }\n");
...
static void a_struct_read_through_its_fields_keeps_its_slots(void) {
    Allocated a;
    allocate(&a, "struct P { x: int, y: int }\n"
...
static void an_array_filled_element_by_element_keeps_its_slots(void) {
    Allocated a;
    allocate(&a, "func one(): int { let x: array<int, 3> = [1, 20, 300]; return x[1]; }\n");
