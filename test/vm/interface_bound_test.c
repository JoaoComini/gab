...
static void test_a_bound_lets_the_body_call_what_the_interface_declares() {
    assert(compiles_with("func total<T: Countable>(x: &T): i32 { return x.count(); }\n"));
}

static void test_a_body_calling_what_no_bound_declares_is_refused() {
    assert(!compiles_with("func total<T: Countable>(x: &T): i32 { return x.missing(); }\n"));
}

static void test_an_unbounded_parameter_has_no_methods() {
    assert(!compiles_with("func total<T>(x: &T): i32 { return x.count(); }\n"));
}

static void test_a_bounded_call_runs_on_an_implementor() {
    assert(runs_with("func total<T: Countable>(x: &T): i32 { return x.count(); }\n"
                     "func main(): i32 {\n"
                     "    let b = Bag { n: 6 };\n"
                     "    return total(b);\n"
                     "}\n"
...
static void test_a_type_that_does_not_implement_is_refused_at_the_call() {
    assert(!compiles_with("struct Plain { m: i32 }\n"
...
static void test_a_type_supplying_the_methods_without_declaring_it_is_refused() {
    assert(!compiles_with("struct Sneak { n: i32 }\n"
                          "impl Sneak { func count(self: &Sneak): i32 { return self.n; } }\n"
                          "func total<T: Countable>(x: &T): i32 { return x.count(); }\n"
                          "func main(): i32 {\n"
                          "    let s = Sneak { n: 1 };\n"
                          "    return total(s);\n"
...
static void test_a_bound_names_an_interface() {
    assert(!compiles_with("func total<T: Bag>(x: &T): i32 { return 0; }\n"));
    assert(!compiles_with("func total<T: Missing>(x: &T): i32 { return 0; }\n"));
}

static void test_a_bound_is_checked_once_not_per_instantiation() {
    assert(!compiles_with("func total<T: Countable>(x: &T): i32 { return x.count() + x.missing(); }\n"));
}

static void test_a_generic_body_is_checked_though_nothing_calls_it() {
    assert(!compiles_with("func total<T>(x: &T): i32 { return x.anything_at_all(); }\n"));
    assert(!compiles_with("func total<T>(x: &T): i32 { let n: i32 = \"text\"; return n; }\n"));
}

static void test_checking_a_body_does_not_emit_a_specialization() {
    TestProgram program = test_compile("func id<T>(x: T): T { return x; }\n"
                                       "func main(): i32 { return id(1); }\n"
                                       "let r: i32 = main();");
...
static void test_an_error_in_a_generic_body_is_reported_once() {
    assert(test_diagnostic_count("func bad<T>(x: T): i32 { let n: i32 = \"text\"; return n; }\n"
                                 "func main(): i32 {\n"
                                 "    let a: i32 = bad<i32>(1);\n"
                                 "    let b: i32 = bad<f32>(2.5);\n"
                                 "    return a + b;\n"
                                 "}\n") == 1);
}
...
static void test_a_generic_that_instantiates_itself_without_end_is_refused() {
    assert(test_diagnostic_mentions("struct Box<T> { value: T }\n"
                                    "func f<T>(x: &T): i32 { let b = Box<T> { value: *x }; return f(b); }\n"
                                    "func main(): i32 { let n: i32 = 0; return f(n); }\n",
                                    "without end"));
}
