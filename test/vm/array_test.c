...
static void test_an_array_of_an_owning_element_does_not_copy() {
    assert(!test_compiles("struct Box { n: int }\n"
                          "func f(): int {\n"
                          "    let xs: Array box Box,1;\n"
                          "    let ys: Array box Box,1 = xs;\n"
                          "    return 0;\n"
                          "}\n"));
}
...
static void test_an_array_names_its_element_and_its_length() {
    ...
    assert(!test_compiles("func f(xs: Array int): int { return 0; }\n"));

    assert(test_compiles("func f(xs: ref Array int,3): int { return 0; }\n"));
}

// An array lends to a borrow like anything else, and the borrow indexes what
// the caller holds.
static void test_an_array_lends_to_a_borrow() {
    assert(test_run_int("func g(xs: ref Array int,3): int { return xs[0]; }\n"
