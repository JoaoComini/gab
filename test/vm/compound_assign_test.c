...
static void test_add_assign() {
    assert(test_run_int("func f(): i32 { let a: i32 = 4; a += 3; return a; }\n"
                        "let r: i32 = f();\n") == 7);
}
...
static void test_div_assign() {
    assert(test_run_int("func f(): i32 { let a: i32 = 12; a /= 4; return a; }\n"
                        "let r: i32 = f();\n") == 3);
}

static void test_mod_assign() {
    assert(test_run_int("func f(): i32 { let a: i32 = 17; a %= 5; return a; }\n"
                        "let r: i32 = f();\n") == 2);
}

static void test_the_target_is_the_left_operand() {
    assert(test_run_int("func f(): i32 { let a: i32 = 10; a -= 4; return a; }\n"
                        "let r: i32 = f();\n") == 6);

    assert(test_run_int("func f(): i32 { let a: i32 = 20; a /= 5; return a; }\n"
                        "let r: i32 = f();\n") == 4);
}

static void test_the_operand_is_a_whole_expression() {
    assert(test_run_int("func f(): i32 { let a: i32 = 1; a += 2 * 3; return a; }\n"
                        "let r: i32 = f();\n") == 7);

    assert(test_run_int("func f(): i32 { let a: i32 = 1; let b: i32 = 4; a += b - 1; return a; }\n"
                        "let r: i32 = f();\n") == 4);
}

static void test_compound_assign_on_a_float() {
    assert(test_run_float("func f(): f32 { let a: f32 = 1.5; a += 2.0; return a; }\n"
                          "let r: f32 = f();\n") == 3.5f);
}

static void test_compound_assign_reaches_a_field() {
    assert(test_run_int("struct Point { x: i32, y: i32 }\n"
                        "func f(): i32 { let v = Point { x: 4, y: 0 }; v.x += 3; return v.x; }\n"
                        "let r: i32 = f();\n") == 7);

    assert(test_run_int("struct Point { x: i32, y: i32 }\n"
                        "func f(): i32 { let v = Point { x: 10, y: 0 }; v.x -= 4; return v.x; }\n"
                        "let r: i32 = f();\n") == 6);
}

static void test_compound_assign_reaches_through_a_pointer() {
    assert(test_run_int("func f(): i32 { let a: i32 = 4; let p: &i32 = a; *p += 3; return a; }\n"
                        "let r: i32 = f();\n") == 7);

    assert(test_run_int("func f(): i32 { let a: i32 = 10; let p: &i32 = a; *p -= 4; return a; }\n"
                        "let r: i32 = f();\n") == 6);
}

static void test_compound_assign_inherits_the_operator_type_rule() {
    assert(test_compiles("func f(): i32 { let a: i32 = 1; a %= 2; return a; }\n"));

    assert(!test_compiles("func f(): f32 { let a: f32 = 1.0; a %= 2.0; return a; }\n"));
    assert(!test_compiles("func f(): bool { let a: bool = true; a += true; return a; }\n"));
}

static void test_div_assign_by_zero_traps() {
    assert(test_run_status("func f(): i32 { let a: i32 = 1; let b: i32 = 0; a /= b; return a; }\n"
                           "let r: i32 = f();\n") == VM_RUN_ERR_DIVIDE_BY_ZERO);
}

static void test_the_target_is_evaluated_once() {
    assert(test_run_int("func bump(n: &i32, p: &i32): &i32 { *n += 1; return p; }\n"
                        "func f(): i32 { let seen: i32 = 0; let a: i32 = 4;\n"
                        "                *bump(seen, a) += 3;\n"
                        "                return seen; }\n"
                        "let r: i32 = f();\n") == 1);
}
...
static void test_a_call_in_the_target_still_assigns() {
    assert(test_run_int("func pick(p: &i32): &i32 { return p; }\n"
                        "func f(): i32 { let a: i32 = 4; *pick(a) += 3; return a; }\n"
                        "let r: i32 = f();\n") == 7);
}

static void test_compound_assign_is_not_an_expression() {
    assert(!test_compiles("func f(): i32 { let a: i32 = 1; let b: i32 = (a += 1); return b; }\n"));
}

static void test_the_target_must_be_assignable() {
    assert(!test_compiles("func f(): i32 { let a: i32 = 1; -a += 1; return a; }\n"));
    assert(!test_compiles("func f(): i32 { let a: i32 = 1; 2 += a; return a; }\n"));
}
