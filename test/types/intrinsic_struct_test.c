...
static void a_unique_owns_what_it_points_at(void) {
    ...
    Resolution declared = scope_resolve(scope, type_registry_names(registry)->unique);
    ...
    const Type *plain = scope_type_lookup(scope, string_from_cstr(&ctx.strings, "Plain"));
