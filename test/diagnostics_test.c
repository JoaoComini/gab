...
// Runs the full front end (parse + resolve) over a source string and returns
// the diagnostics it produced. The arena keeps the messages alive.
static void compile(Arena *arena, Diagnostics *diagnostics, const char *source) {
...
static void test_poison_suppresses_cascades() {
    ...
    compile(arena, &diagnostics, "func test() { let x: int = undefined_var + 1; }");
...
static void test_reports_multiple_semantic_errors() {
    ...
    compile(arena, &diagnostics, "func test() { let a: int = first; let b: int = second; }");
...
static void test_reports_type_mismatch() {
    ...
    compile(arena, &diagnostics, "func test() { let x: int = 1.5; }");
...
static void test_reports_unknown_type() {
    ...
    compile(arena, &diagnostics, "func test() { let v: Vec3 = 1; }");
...
static void test_reports_duplicate_declaration() {
    ...
    compile(arena, &diagnostics, "func test() { let x: int = 1; let x: int = 2; }");
...
static void test_parser_recovers_across_statements() {
    ...
    compile(arena, &diagnostics, "func test() { let ; let ; }");
...
static void test_syntax_errors_name_the_found_token() {
    ...
    compile(arena, &diagnostics, "func test() { let x: int = 1 }");
...
static void test_spans_track_lines_and_columns() {
    ...
    compile(arena, &diagnostics, "func test() {\n    let x: int = 1;\n    let y: int = nope;\n}");
