...
static void test_type_survives_a_later_compile(void) {
    ...
    bool second = gab_vm_load(vm, "<second>",
                              "module test;\n"
                              "func noop() { }\n",
                              &err);
    assert(second);
    ...
    assert(gab_type_field_offset(vm, player, "health", &health_offset));
...
static void test_layout_matches_c(void) {
    ...
    assert(gab_type_field_offset(vm, type, "health", &offset));
    assert(offset == offsetof(Player, health));

    assert(gab_type_field_offset(vm, type, "mana", &offset));
    assert(offset == offsetof(Player, mana));

    assert(!gab_type_field_offset(vm, type, "stamina", &offset));
...
static void test_lookup_failures(void) {
    ...
    assert(gab_vm_lookup(vm, "test", "missing", &err) == NULL);
    assert(err.message[0] != '\0');

    assert(gab_vm_lookup(vm, "test", "notafunc", &err) == NULL);
    assert(err.message[0] != '\0');

    GabFunc *fn = gab_vm_lookup(vm, "test", "real", &err);
    assert(fn);
...
static void test_a_host_call_reaches_a_builtin_method(void) {
    ...
    GabFunc *fn = gab_vm_lookup(vm, "test", "size", &err);
    assert(fn);
...
static void test_call_with_scalar_args(void) {
    ...
    GabFunc *fn = gab_vm_lookup(vm, "test", "mix", &err);
    assert(fn);
    ...
    gab_call_int(fn_call, 0, 21);
    gab_call_float(fn_call, 1, 1.5f);
    gab_call_bool(fn_call, 2, true);

    int result = 0;
    assert(gab_call(vm, fn_call, &result, &err) == GAB_OK);
    assert(result == 42);

    gab_call_bool(fn_call, 2, false);
    assert(gab_call(vm, fn_call, &result, &err) == GAB_OK);
    assert(result == 21);
...
static void test_call_in_a_loop(void) {
    ...
    GabFunc *fn = gab_vm_lookup(vm, "test", "step", &err);
    assert(fn);
...
static void test_struct_argument_and_return(void) {
    ...
    GabFunc *fn = gab_vm_lookup(vm, "test", "hurt", &err);
    assert(fn);
    ...
    gab_call_struct(fn_call, 0, &in, sizeof(in));
    gab_call_int(fn_call, 1, 25);
...
static void test_bad_arguments_are_rejected(void) {
    ...
    GabFunc *fn = gab_vm_lookup(vm, "test", "take", &err);
    assert(fn);

    GabCall *fn_call = gab_call_init(fn, &err);
    assert(fn_call);

    assert(!gab_call_int(fn_call, 7, 1));
    assert(gab_call(vm, fn_call, NULL, &err) == GAB_ERR_ARG);
    assert(err.message[0] != '\0');

    assert(!gab_call_float(fn_call, 1, 1.0f));
    assert(gab_call(vm, fn_call, NULL, &err) == GAB_ERR_ARG);

    int not_a_player = 0;
    assert(!gab_call_struct(fn_call, 0, &not_a_player, sizeof(not_a_player)));
    assert(gab_call(vm, fn_call, NULL, &err) == GAB_ERR_ARG);

    Player p = {.health = 1, .mana = 2};
    assert(gab_call_struct(fn_call, 0, &p, sizeof(p)));
    assert(gab_call_int(fn_call, 1, 99));
...
static void test_runtime_error_is_reported(void) {
    ...
    GabFunc *fn = gab_vm_lookup(vm, "test", "boom", &err);
    assert(fn);

    GabCall *fn_call = gab_call_init(fn, &err);
    assert(fn_call);

    gab_call_int(fn_call, 0, 1);
    ...
    GabFunc *good = gab_vm_lookup(vm, "test", "fine", &err);
    assert(good);

    GabCall *good_call = gab_call_init(good, &err);
    assert(good_call);

    gab_call_int(good_call, 0, 41);
...
static void test_unset_arguments_are_rejected(void) {
    ...
    GabFunc *fn = gab_vm_lookup(vm, "test", "two", &err);
    assert(fn);
    ...
    gab_call_int(fn_call, 1, 1000);
    assert(gab_call(vm, fn_call, &result, &err) == GAB_OK);
    assert(result == 1001);

    gab_call_int(fn_call, 0, 2);
    assert(gab_call(vm, fn_call, &result, &err) == GAB_OK);
    assert(result == 1002);
...
static void test_rejected_setter_does_not_supply_an_argument(void) {
    ...
    assert(gab_call_int(fn_call, 0, 5));
    ...
    assert(gab_call_int(fn_call, 1, 7));
    assert(gab_call(vm, fn_call, &result, &err) == GAB_OK);
    assert(result == 12);

    assert(!gab_call_int(fn_call, 9, 1));
    assert(!gab_call_int(fn_call, -1, 1));
...
static void test_two_modules_can_share_a_function_name(void) {
    ...
    GabFunc *player_update = gab_vm_lookup(vm, "Player", "on_update", &err);
    GabFunc *enemy_update = gab_vm_lookup(vm, "Enemy", "on_update", &err);
...
static void test_the_script_names_its_module_not_the_filename(void) {
    ...
    assert(gab_vm_lookup(vm, "some_file", "f", &err) == NULL);
    assert(gab_vm_lookup(vm, "test", "f", &err) == NULL);
    ...
    GabFunc *g = gab_vm_lookup(vm, "test", "g", &err);
    assert(g);
...
static void test_lookup_by_module_name(void) {
    ...
    assert(gab_vm_lookup(vm, "Nope", "hp", &err) == NULL);
    assert(err.message[0] != '\0');

    assert(gab_vm_lookup(vm, "Enemy", "missing", &err) == NULL);
...
static void test_two_modules_can_share_a_type_name(void) {
    ...
    const GabType *enemy_config = gab_vm_find_type(vm, "Enemy", "Config");
    ...
    size_t offset = SIZE_MAX;
    assert(gab_type_field_offset(vm, player_config, "mana", &offset));
    assert(offset == sizeof(int));

    assert(!gab_type_field_offset(vm, enemy_config, "mana", &offset));
    assert(gab_type_field_offset(vm, enemy_config, "hp", &offset));

    assert(gab_vm_find_type(vm, "Player", "Config") == player_config);
    assert(gab_vm_find_type(vm, "Enemy", "Config") == enemy_config);
...
static void test_builtins_are_shared_across_modules(void) {
    ...
    GabFunc *player_size = gab_vm_lookup(vm, "Player", "player_size", &err);
    GabFunc *enemy_size = gab_vm_lookup(vm, "Enemy", "enemy_size", &err);
...
static void test_module_type_shadows_the_root(void) {
    ...
    assert(gab_vm_find_type(vm, "Nope", "Config") == NULL);
...
static void test_qualified_type_reference_crosses_modules(void) {
    ...
    assert(gab_type_size(vm, gab_vm_find_type(vm, "Enemy", "Config")) == sizeof(int));
...
static void test_a_method_is_not_reachable_from_a_host(void) {
    ...
    assert(!gab_vm_lookup(vm, "M", "hp", &err));
...
static void test_the_vm_owns_its_handles(void) {
    ...
    for (int i = 0; i < 9; i++) {
        handles[i] = gab_vm_lookup(vm, "test", names[i % 3], &err);
        assert(handles[i]);
    ...
    for (int i = 0; i < 9; i++) {
        assert(gab_call_int(calls[i], 0, 10));
...
static void test_two_callers_stage_independently(void) {
    ...
    GabFunc *fn = gab_vm_lookup(vm, "test", "damage", &err);
    assert(fn);
    ...
    assert(gab_call_int(ai, 0, 100));

    assert(gab_call_int(projectile, 0, 40));
    assert(gab_call_int(projectile, 1, 10));

    int result = -1;
    assert(gab_call(vm, projectile, &result, &err) == GAB_OK);
    assert(result == 30);

    assert(gab_call_int(ai, 1, 5));

    result = -1;
    assert(gab_call(vm, ai, &result, &err) == GAB_OK);
    assert(result == 95);

    assert(gab_call_int(projectile, 1, 1));
...
static void test_a_host_pointer_reaches_a_script() {
    ...
    size_t health_at = 0;
    assert(gab_type_field_offset(vm, player, "health", &health_at));
    ...
    assert(gab_call_pointer(call, 0, object, player));
    assert(gab_call_int(call, 1, 40));
...
static void test_a_pointer_argument_checks_its_pointee() {
    ...
    const GabType *player = gab_vm_find_type(vm, "test", "Player");
    const GabType *enemy = gab_vm_find_type(vm, "test", "Enemy");
    ...
    assert(!gab_call_pointer(call, 0, object, enemy));
...
static void test_a_type_reports_the_layout_a_host_allocates_by() {
    ...
    size_t mana_at = 0;
    assert(gab_type_field_offset(vm, player, "mana", &mana_at));
